...
async function generateRemedialAction(topKPI, underPerf) {
  const prompt = `
  Based on these inputs, suggest remedial actions:
  Top KPI (strong areas): ${topKPI}
  Underperforming KPIs (weak areas): ${underPerf}

  Provide a short professional remedial action in bullet points.
  `;
  ...
  try {
    ...
    return "⚠️ Error generating remedial action.";
  }
}
...
generateBtn.addEventListener('click', async () => {
  ...
  // Call AI for remedial action
  const remedialText = await generateRemedialAction(
    topKPIEl.value.trim(),
    underPerfEl.value.trim()
  );
